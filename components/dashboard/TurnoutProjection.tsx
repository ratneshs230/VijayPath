...
const TurnoutProjection: React.FC<TurnoutProjectionProps> = ({
  ...
  const categories = [
    ...
    {
      label: t.dashboard.mediumTurnout,
  ...
  return (
    <div className="bg-slate-800 rounded-2xl p-6">
      ...
      <div className="text-center mb-6 p-4 bg-slate-700/50 rounded-xl">
        ...
        <div className="text-sm text-slate-400">{t.dashboard.expectedToVote}</div>
        <div className="text-xs text-slate-500 mt-1">
          {expectedTurnoutPercent}% {t.dashboard.ofRegistered} {totalVoters}
        </div>
      </div>
      ...
      <div className="mt-6 pt-4 border-t border-slate-700">
        <div className="flex justify-between text-sm">
          <span className="text-slate-400">{t.dashboard.presentInVillageLabel}</span>
          <span className="text-white font-medium">{totalPresentVoters}</span>
        </div>
        <div className="flex justify-between text-sm mt-2">
          <span className="text-slate-400">{t.dashboard.awayMigrated}</span>
